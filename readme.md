...
estsb was originally named Simplicity — a term that described exactly what the project was all about. However, that was prior to the discovery of a [WordPress](http://wordpress.org/) theme already using that very name. After a brief discussion, several Latin terms were bandied about as replacements. estsb (which means ‘clean, neat, or cleansed’) seemed to fit the bill rather nicely — and here we are.

A version of estsb is being developed for [Tumblr](http://estsbtheme.tumblr.com/) and is also [available on GitHub](http://github.com/splorp/estsb-tumblr/).

Keep track of estsb development on [Twitter](http://twitter.com/estsbtheme).
...
By removing the majority of the presentational markup and other cruft from the estsb parent theme, we’ve opened the door for child themes to pick up the aesthetic slack. There are currently two child themes being developed for estsb.
...
The estsb theme is absolutely free and conveniently licensed under GPL. You may use it for personal or commercial projects, as you see fit. Please refer to the [license.txt](https://github.com/splorp/estsb/blob/master/license.txt) file included with the source for more information.
...
+ Fixed error where `$estsb_body_class` wasn’t defined in `functions.php`
...
+ Namespaced all theme functions with the `estsb_` prefix
