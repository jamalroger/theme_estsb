...
if ( ! function_exists( 'estsb_add_admin' ) ) {
	function estsb_add_admin() {
		...
		add_theme_page($theme_name_full." Options", "".$theme_name_full." Options", 'edit_themes', basename(__FILE__), 'estsb_admin');
	}
}

if ( ! function_exists( 'estsb_admin' ) ) {
	function estsb_admin() {
		global $theme_name_full, $theme_name, $options;
...
if ( ! function_exists( 'estsb_body_class' ) ) {
	function estsb_body_class($wp_class_list, $simple_class_list) {
	...
	add_filter('body_class', 'estsb_body_class', 10, 2);
}


// Replace default post class verbosity

if ( ! function_exists( 'estsb_post_class' ) ) {
	function estsb_post_class() {
		global $post;
		$c = array();
	...
	add_filter( 'post_class', 'estsb_post_class' );
}


// Remove non-validating rel attributes from category links

if ( ! function_exists( 'estsb_relfix' ) ) {
	function estsb_relfix($c) {
		return preg_replace('/category tag/','tag',$c);
	}

	add_filter('the_category','estsb_relfix');
}
...
if ( ! function_exists( 'estsb_remove_style' ) ) {
	function estsb_remove_style() {
		global $wp_widget_factory;
		remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style'  ) );
	}

	add_action( 'widgets_init', 'estsb_remove_style' );
}


// Add a proper thousands delimiter to category post counts

if ( ! function_exists( 'estsb_delim' ) ) {
	function estsb_delim($c) {
		return preg_replace('/(\d)(\d{3})\b/','\1,\2',$c);	// Hat tip to MyFonts for the regex tweaks
	}

	add_filter('wp_list_categories','estsb_delim');
}


// Replace single attribute quotes with double quotes

if ( ! function_exists( 'estsb_double_down' ) ) {
	function estsb_double_down($c) {
		return preg_replace('/\'/','"',$c);
	}

	add_filter('wp_tag_cloud','estsb_double_down');
	add_filter('wp_list_categories','estsb_double_down');
	add_filter('get_archives_link','estsb_double_down');
	add_filter('get_comment_author_link','estsb_double_down');
	add_filter('next_image_link','estsb_double_down');
	add_filter('previous_image_link','estsb_double_down');
	...
	if ( ! is_admin() ) {
		add_filter('get_avatar','estsb_double_down');
	}
}
...
if ( ! function_exists( 'estsb_decruft' ) ) {
	function estsb_decruft($c) {
		$c_ = preg_replace('/ class=[\"\'].+?[\"\']/','',$c);
		return preg_replace('/ id=[\"\'].+?[\"\']/','',$c_);
	}

	add_filter('wp_tag_cloud','estsb_decruft');
	add_filter('wp_list_bookmarks','estsb_decruft');
	add_filter('wp_list_categories','estsb_decruft');
	add_filter('wp_list_pages','estsb_decruft');
	add_filter('wp_nav_menu','estsb_decruft');
	add_filter('wp_page_menu','estsb_decruft');
	add_filter('edit_comment_link','estsb_decruft');
	add_filter('comment_reply_link','estsb_decruft');
}


// Remove crufty class and ID attributes from tag cloud, reformat title attributes

if ( ! function_exists( 'estsb_decruft_tagcloud' ) ) {
	function estsb_decruft_tagcloud($c) {
		$c_ = preg_replace('/ style=[\"\'].+?[\"\']/','',$c);
		return preg_replace('/ title=[\"\']([0-9]+?) topic(s?)[\"\']/',' title="View \1 post\2"',$c_);
	}

	add_filter('wp_tag_cloud','estsb_decruft_tagcloud');
}


// Remove crufty class attributes from avatars

if ( ! function_exists( 'estsb_decruft_avatar' ) ) {
	function estsb_decruft_avatar($c) {
		return preg_replace('/ class=[\"\'].+?[\"\']/',' class="photo"',$c);
	}

	if ( ! is_admin() ) {	// Don't apply filter to admin pages
		add_filter('get_avatar','estsb_decruft_avatar');
	}
}
...
if ( ! function_exists( 'estsb_decruft_comment_form' ) ) {
	function estsb_decruft_comment_form($c) {
		$find = array(
...
if ( ! function_exists( 'estsb_gallery' ) ) {
	function estsb_gallery($attr) {
		global $post;
	...
	add_shortcode('gallery', 'estsb_gallery');
}
...
if ( ! function_exists( 'estsb_comment' ) ) {
	function estsb_comment($comment, $args, $depth) {
...
if ( ! function_exists( 'estsb_rss_thumb' ) ) {
	function estsb_rss_thumb($content) {
	   global $post;
	...
	add_filter('the_excerpt_rss', 'estsb_rss_thumb');
	add_filter('the_content_feed', 'estsb_rss_thumb');
}
...
if ( ! function_exists( 'estsb_excerpt_more' ) ) {
	function estsb_excerpt_more($more) {
		global $post;
		$t = get_post($post->ID);
		$title = $t->post_title;
	...
	add_filter('excerpt_more', 'estsb_excerpt_more');
}


// Custom title element text

if ( ! function_exists( 'estsb_title' ) ) {
	function estsb_title( $title, $sep ) {
		global $paged, $page;
